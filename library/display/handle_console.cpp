...
void console::move_to::top() {
    ...
    int pos_x = WORK_AREA_SIZE.cx / 2
                - (rect_window.right - rect_window.left) / 2
                + OFFSET_MIDDLE_COLUMN,
        pos_y = 0;
    move_to_xy(pos_x, pos_y);
}
void console::move_to::top_right() {
    ...
    HWND hwnd = GetConsoleWindow();
    GetWindowRect(hwnd, &rect_window);
    int pos_x = WORK_AREA_SIZE.cx - (rect_window.right - rect_window.left)
                + OFFSET_RIGHT,
        pos_y = 0;
    move_to_xy(pos_x, pos_y);
}
...
void console::move_to::left() {
    ...
    HWND hwnd = GetConsoleWindow();
    GetWindowRect(hwnd, &rect_window);
    int pos_x = 0, pos_y = WORK_AREA_SIZE.cy / 2
                           - (rect_window.bottom - rect_window.top) / 2
                           + OFFSET_MIDDLE_ROW;
    move_to_xy(pos_x, pos_y);
}
...
void console::move_to::bottom_left() {
    ...
    HWND hwnd = GetConsoleWindow();
    GetWindowRect(hwnd, &rect_window);
    int pos_x = 0, pos_y = WORK_AREA_SIZE.cy
...
void console::move_to::bottom() {
    ...
    int pos_x = WORK_AREA_SIZE.cx / 2
                - (rect_window.right - rect_window.left) / 2
                + OFFSET_MIDDLE_COLUMN,
        pos_y = WORK_AREA_SIZE.cy - (rect_window.bottom - rect_window.top)
                + OFFSET_BOTTOM;
    move_to_xy(pos_x, pos_y);
}
...
void console::resize(short width, short height) {
    ...
    MoveWindow(
        ...
        rect_window.left,
        rect_window.top,
        width + point_offset.x,
        height + point_offset.y,
        TRUE
    );
}
